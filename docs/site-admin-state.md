...
## 2026-04-20 chatbot knowledge-base + Spark retrieval rollout

### Latest milestone

- GasGx public-site chatbot is now wired as:
  - website widget in `shared/ui/site-shell.shared.js`
  - Supabase Edge Function `site-chat`
  - XFYUN Spark generation
  - Supabase-hosted policy rules + knowledge retrieval
- The retrieval stack now includes:
  - `knowledge_documents`
  - `knowledge_chunks`
  - `chat_faq_rules`
  - `chat_qa_logs`
  - `chat_lead_intents`
- A first production knowledge layer was seeded with:
  - site snapshot content
  - deployment-specific product cards
  - stranded-gas mining country playbooks for the United States, Canada, and Russia
  - stranded-gas quotation checklist
  - stranded-gas promise-boundary knowledge

### Effective behaviors

- The chatbot no longer depends on localhost or a page-only prompt.
- High-frequency questions can now resolve through three stable paths:
  - `gasgx_policy`
  - `gasgx_rag`
  - `xfyun_spark`
- Public-site calls use the deployed Supabase function endpoint and the publishable key already present in the site shell.
- Country-specific stranded-gas mining questions for:
  - United States
  - Canada
  - Russia
  now resolve to deterministic GasGx policy answers backed by internal sales knowledge-card sources.

### Solved in this thread

- Added `site-chat` retrieval/policy orchestration in `supabase/functions/site-chat/index.ts`.
- Added initial knowledge-base migrations under `supabase/migrations/` for:
  - retrieval schema
  - locale seed fix
  - site snapshot ingestion
  - deployment cards
  - stranded-gas country cards
  - stranded-gas country FAQ rules
  - stranded-gas sales playbook
- Added the minimal admin knowledge module:
  - `article_management/modules/chat-knowledge-admin.module.js`
- Confirmed online deployment:
  - Supabase function deployed successfully
  - remote DB migrations applied
  - public shell file uploaded and reachable on `www.gasgx.com`
- Verified live chatbot cases for:
  - FAQ
  - deployment pages
  - ECM page
  - stranded-gas country guidance

### Unfinished

- The chatbot knowledge backend is live, but the admin navigation/bootstrap integration for the new knowledge module is still entangled with other in-progress sales/admin changes in the working tree.
- Product `container` deployment retrieval still uses a broad second source in some cases because the knowledge base needs even finer deployment/container-specific chunks.
- Stranded-gas quotation questions still tend to hit the older general quote rule before a dedicated stranded-gas quotation rule is fully prioritized.
